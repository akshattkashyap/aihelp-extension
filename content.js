...
(function () {
  ...
  /**
   * Markdown to HTML converter — handles headings, lists, tables, hr, bold, italic, code
   */
  function parseMarkdown(text) {
    // Helper: escape HTML entities in a string
    function esc(s) {
      return s
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
    ...
    // Helper: apply inline formatting (bold, italic, code) to an already-escaped line
    function inlineFmt(line) {
      return (
        line
          // Inline code (before bold/italic so backtick content stays untouched)
          .replace(
            /`(.+?)`/g,
          ...
          // Italic: *text* or _text_  (single * or _ that aren't part of **)
          .replace(/\*(.+?)\*/g, "<em>$1</em>")
          .replace(/(?<!\w)_(.+?)_(?!\w)/g, "<em>$1</em>")
      );
    }

    // Helper: check if a line is a table row (starts and ends with |, or starts with |)
    ...
    while (i < lines.length) {
      ...
      // --- Blank line → skip (ends current implicit paragraph) ---
      if (trimmed === "") {
        i++;
        continue;
      }
      ...
      if (isTableRow(trimmed)) {
        ...
        if (tableRows.length >= 2) {
          html.push(
            '<table style="border-collapse:collapse;margin:10px 0;width:100%;font-size:13px;">',
          );
          ...
          for (const cell of headerCells) {
            html.push(
              `<th style="border:1px solid #ddd;padding:8px 10px;background:#f5f5f5;font-weight:600;text-align:left;color:#1a1a1a;">${inlineFmt(esc(cell))}</th>`,
            );
          }
          html.push("</tr></thead>");

          // Determine start of body rows (skip separator row if present)
          let bodyStart = 1;
          ...
          if (bodyStart < tableRows.length) {
            ...
            for (let r = bodyStart; r < tableRows.length; r++) {
              if (isTableSeparator(tableRows[r])) continue; // skip extra separators
              const cells = parseCells(tableRows[r]);
              html.push("<tr>");
              for (const cell of cells) {
                html.push(
                  `<td style="border:1px solid #ddd;padding:8px 10px;color:#444444;">${inlineFmt(esc(cell))}</td>`,
                );
              }
              html.push("</tr>");
            }
          ...
          html.push(
            `<p style="margin:8px 0;line-height:1.6;">${inlineFmt(esc(tableRows[0]))}</p>`,
          );
        }
        continue;
      }
      ...
      if (headingMatch) {
        ...
        html.push(
          `<h${level} style="font-size:${sizes[level]};font-weight:600;color:#1a1a1a;margin:${margins[level]};line-height:1.3;">${inlineFmt(esc(headingMatch[2]))}</h${level}>`,
        );
        i++;
        continue;
      }
      ...
      if (/^\d+\.\s/.test(trimmed)) {
        ...
        while (i < lines.length && /^\d+\.\s/.test(lines[i].trim())) {
          ...
          html.push(
            `<li style="margin:4px 0;">${inlineFmt(esc(content))}</li>`,
          );
          i++;
        }
        html.push("</ol>");
      ...
      if (paraLines.length > 0) {
        html.push(
          `<p style="margin:8px 0;line-height:1.6;">${inlineFmt(esc(paraLines.join(" ")))}</p>`,
        );
      }
    }
